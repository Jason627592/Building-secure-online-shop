...
<head>
    <meta charset="utf-8">
    <title>Delete Products By Category</title>
...
<body>
    ...
    <main>
        ...
        <form id="del_prod_by_cat" method="POST" action="admin-process.php?action=deleteAllProductByCategory">
            <h2>Delete Products By Category</h2>
            <label for="prod_catid">Category *</label>
            <div>
                <select id="prod_catid" name="delete_category">
                    <?php echo $options; ?>
                </select>
            </div>
            <br>
            <input type="hidden" name="nonce" value="<?php echo htmlspecialchars(csrf_getNonce("deleteAllProductByCategory")); ?>">
